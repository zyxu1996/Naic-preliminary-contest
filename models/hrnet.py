...
class CEM(nn.Module):
    def __init__(self, planes):
        ...
        # self.conv5 = conv1x1(planes + planes // 2, planes // 4)
        # self.dilaconv5 = dilaconv(planes // 4, planes // 8, dilation=6, padding=6)
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, reduction=16, stage4=False):
        ...
        self.stage4 = stage4
        if self.stage4:
            # self.conv_out = nn.Conv2d(planes, planes, kernel_size=3, stride=1,
            #                                 padding=2, bias=False, dilation=2)
            # self.bn_out = BatchNorm2d(planes, momentum=BN_MOMENTUM)
...
class HRNet(nn.Module):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        # Top layer
        channels = cfg.STAGE4['NUM_CHANNELS']
        ...
        self.latlayer3 = nn.Conv2d(channels[0], channels[0], kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x):
        ...
        for i in range(self.stage2_cfg['NUM_BRANCHES']):
            if self.transition1[i] is not None:
                ...
                x_list.append(x)
        # print("x_list:{}".format(x_list[0].shape))
        y_list = self.stage2(x_list)
        # print("stage2:{}".format(y_list[1].shape))
        x_list = []
        ...
        # print("stage3:{}".format(y_list[2].shape))
        x_list = []
        ...
        y3 = F.interpolate(y3, scale_factor=8, mode='bilinear')
        y0 = torch.cat((F.interpolate(y1, scale_factor=2, mode='bilinear'), self.latlayer3(y_list[0]), y3), dim=1)
